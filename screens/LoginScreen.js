...
export default function LoginScreen() {
  ...
  async function onLogin() {
    ...
    const deviceId = Device.osInternalBuildId || Device.androidId || Device.deviceId || Device.modelName || 'unknown-device';
    try {
      const resp = await loginPhone(phone, deviceId);
      if (!resp.ok) {
        return Alert.alert('Login failed', JSON.stringify(resp));
      }
      // resp should include role, token, userId, expiry, refreshToken optionally
  ...
  return (
    <View style={{flex:1,justifyContent:'center',padding:20}}>
      <Text style={{fontSize:22,fontWeight:'700',marginBottom:10}}>Security Ops — Login</Text>
